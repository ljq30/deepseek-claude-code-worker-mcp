...
当前 GitHub beta tag：`v0.3.20-beta.36`。
...
版本 `0.3.20-beta.36` 的原则：汇报动作，不替 Codex 判案。`allowed_dirs` 外的普通变更会作为事实报告，不再自动判定 worker 失败；`forbidden_paths` 仍然是硬失败。`allow_docs_only` 只保留为兼容旧调用的参数，文档变更会被报告，不再因为 docs-only 自动失败。
